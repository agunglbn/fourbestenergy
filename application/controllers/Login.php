...
class Login extends CI_Controller
{
    ...
    //REGISTER/LOGIN  USER 
    public function viewpenggunaLogin()
    {
        if ($this->session->userdata('login')) {
            redirect('dashboardAlum');
        } else {
            $this->load->view('User/login_pengguna');
        }
    }
    public function tampilRegister()
    {
        $this->load->view('User/regis_pengguna');
    }
    function regispengguna()
    {
        $this->load->library('form_validation');
        ...
        $this->form_validation->set_rules(
            'username',
            'Username',
            'required|min_length[5]|max_length[15]|is_unique[tbl_pengguna.username]',
            array(
                'required'      => '%s Tidak Boleh Kosong',
        ...
        if ($this->form_validation->run() == false) {
            ...
            $insertUserData = $this->login_model->regispengguna($data);
